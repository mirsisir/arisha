                        ...
                        <div class="row">
                            <div class="col-4">
                                <div class="form-group" style="position: relative;">
                                    <select type="text" name="employee_id" id="employee_id" class="form-control" >
                                        <option value="">--Select Employee--</option>
                                        @foreach($employees as $employee)
                                            <option value="{{$employee->employee_id}}">{{$employee->employee->fname}}  {{$employee->employee->lname}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row" style="height:350px; overflow-y: auto; ">
                            <div class="col-8" style=" padding-right: 0 !important;">
                                <table class="price-table custom-table" style="">
                                    <tr><th>SL</th><th style="width: 100px;">Employee</th><th>Product</th><th>Qty</th><th>Delete</th></tr>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $('#employee_id').on('change',function(e){
          $.ajaxSetup({
            ...
            $.ajax({
                url:'{{ url('/admin/uniform/collection/get_product') }}',
                method: 'POST',
                dataType: 'JSON',
        ...
        $('#qnt').on('keyup', function(e){
            ...
            if(e.which == 13){
                ...
                if(employee_id == '' || product_id == '' || qnt == ''){

                    alert("Please Fillup All Fields ");
                    return false;
                }

                add_product_to_table(employee_id, employee_name,product_id, product_name, qnt);
        ...
        $('#pur_save').click(function(e){
            ...
            $('.price-table tr td').each(function(){
               ...
               if($(this).attr('data-comid') != ''){comid = $(this).attr('data-comid'); cartData.push(comid);}
               else if($(this).attr('data-comid') == ''){cartData.push("0");}
          ...
          $.ajax({
                url: "{{ URL::route('uniform.save_return_products') }}",
                method: 'post',
                data: formData,
                //data: cartData,
                contentType: false,
                ...
                success: function(data){
                        alert("Old Uniform Collection Done!");
    ...
    function add_product_to_table(employee_id, employee_name,product_id, product_name, qnt){
            $('.price-table').show();

            $('.price-table').append("<tr><td>"+sl+"</td><td data-comid='"+employee_id+"' style='width:200px;' class='name'>"+employee_name+"</td><td data-prodid='"+product_id+"' style='width:200px;' class='name'>"+product_name+"</td><td class='qnt'>"+qnt+"</td><td><i class='delete mdi mdi-delete'></i></td></tr>");
            sl =sl + 1;
    }
